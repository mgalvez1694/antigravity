...
Genera mapas regional y de detalle desde el parquet local.
"""
import os
...
@st.cache_resource(show_spinner="Cargando mapa geológico...")
    ...
    if datos_geo:
        ...
        if st.button("🗺️ Generar mapas geológicos", use_container_width=True):
            ...
            with st.spinner("Generando mapas (esto puede tardar unos segundos)..."):
                p_20, p_5, p_2, p_reg, p_det, p_prov = generar_mapa_geologico(
            ...
            st.session_state["mapa_geo_provincial"] = p_prov
